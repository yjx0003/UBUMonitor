...
public class CourseWS {
	 ...
	 * @throws Exception
	 */
	/*public static void setGradeReportLinesNewWebService(String token, int userId, Course course) throws Exception {
		CloseableHttpClient httpclient = HttpClients.createDefault();
		ArrayList<GradeReportLine> children = new ArrayList<>();
		ArrayList<GradeReportLine> actualGRL = new ArrayList<>();
		...
		try {
			...
			try {
				...
				if (jsonArray != null) {
					JSONArray tables = (JSONArray) jsonArray.get("usergrades");
					...
					for (int i = 0; i < gradeitems.length(); i++) {
						...
						// nivel del itemname
						String nameLine = gradeitemsElement.getString("itemname");
						int idLine = gradeitemsElement.getInt("id");
						
						//Obtenemos el tipo del elemento
						String itemType = gradeitemsElement.getString("itemtype");
						int categoryId = 0;
						int itemInstance = 0;
						
						//
						if (itemType.equals("mod")) {
							itemType = gradeitemsElement.getString("itemmodule");
							categoryId = gradeitemsElement.getInt("categoryid");
						} else {
							itemInstance = gradeitemsElement.getInt("itemInstance");
							categoryId = itemInstance;
						}
						
						boolean typeLine = true;
						...
						// "-" si no hay
						String percentage = gradeitemsElement.getString("percentajeformatted");
						
						// Sacamos el peso
						
						// Sacamos el rango
						String gradeMin = gradeitemsElement.getString("grademin");
						String gradeMax = gradeitemsElement.getString("grademax");
						
						GradeReportLine actualLine = new GradeReportLine(idLine, nameLine, categoryId,
								typeLine, 0, gradeMin, gradeMax, grade, percentage, itemType);
						
						
						if(!itemType.equals("Category") && !itemType.equals("Course")) {
							if(lastElementType.equals("Category")) {
								
							}
							children.add(0, actualLine);
						} else if(itemType.equals("Category")) {
							if(lastElementType.equals("Category")) {
								ac
							}
							actualLine.setChildren(children);
							actualGRL.add(actualLine);
						}
