...
public class FirstGroupBy<E extends Serializable, T extends Serializable> implements Serializable {
	...
	public FirstGroupBy(GroupByAbstract<T> groupBy, List<LogLine> logLines, Predicate<LogLine> filter, Function<LogLine, E> getEFunction,
	...
	public void setCounts(List<LogLine> logLines, Predicate<LogLine> filter, Function<LogLine, E> getEFunction,
		...
		counts = logLines.stream()
				...
				.collect(Collectors.groupingBy(LogLine::getUser,
						Collectors.groupingBy(getEFunction,
								Collectors.groupingBy(getTFunction, Collectors.counting()))));
	}
	 ...
	 * @param elements
	 *            primer listado de agrupamiento
	 * @param groupByRange
	 ...
	 */
	public Map<E, List<Double>> getMeans(List<EnrolledUser> enrolledUsers, List<E> elements, LocalDate start, LocalDate end) {
		...
		for (E element : elements) {
			...
			Map<T, DescriptiveStatistics> statisticsMap = statistics.computeIfAbsent(element,
					k -> new HashMap<>());
	 ...
	 */
	public Map<EnrolledUser, Map<E, List<Long>>> getUsersCounts(List<EnrolledUser> users,
		...
		for (EnrolledUser user : users) {
			Map<E, List<Long>> elementsCount = result.computeIfAbsent(user,
					k -> new HashMap<>());
			Map<E, Map<T, Long>> userCounts = counts.computeIfAbsent(user,
					k -> new HashMap<>());
