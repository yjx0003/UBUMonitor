...
public class GroupBoxPlot extends Chartjs {
	...
	public void update() {
		...
		webViewChartsEngine.executeScript(String.format("updateChartjs(%s,%s)", dataset, "boxplotGroupOptions"));
	...
	public String createDataset(List<EnrolledUser> selectedUser, List<GradeItem> selectedGradeItems, Stats stats,
		...
		if(withGroupMean) {
			for (Group group: slcGroup.getItems()) {
				if (group!= null) {
					createData(group.getEnrolledUsers(), selectedGradeItems, stringBuilder, group.getGroupName(), !Buttons.getInstance().getShowGroupMean());
				}
