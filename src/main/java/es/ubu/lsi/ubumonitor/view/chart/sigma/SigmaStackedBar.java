...
public class SigmaStackedBar extends Plotly {
	...
	public void createData(JSArray data) {
		...
		Comparator<Student> comparator = Comparator.comparing(Student::getNumberCreditsPassed, Comparator.reverseOrder())
		...
		List<Integer> creditsPassed = students.stream().map(Student::getNumberCreditsPassed).collect(Collectors.toList());
		List<Integer> basicAndObligatory = students.stream().map(Student::getBasicAndObligatoryCreditsEnrolled).collect(Collectors.toList());
		List<Integer> optionalCredits = students.stream().map(Student::getOptionalCreditsEnrolled).collect(Collectors.toList());
