...
public class SigmaTableEnrolled extends Tabulator {
	private static final Logger LOGGER = LoggerFactory.getLogger(SigmaTableEnrolled.class);
	...
	public SigmaTableEnrolled(MainController mainController, EnrolledUserStudentMapping enrolledUserStudentMapping) {
		super(mainController, ChartType.SIGMA_TABLE_ENROLLED);
	...
	public void exportCSV(String path) throws IOException {
		...
		try (CSVPrinter printer = new CSVPrinter(getWritter(path),
				CSVFormat.DEFAULT.withHeader("userid", FULLNAME, EMAIL, DATE_OF_BIRTH, DNI, GENDER, USUAL_ADDRESS,
	...
	public void update() {
		...
		List<Student> students = enrolledUserStudentMapping.getStudents(enrolledUsers);

		JSArray columns = createColumns();
		JSArray tableData = createData(students);
	...
	private JSArray createData(List<Student> students) {
		...
		for (Student student : students) {
			EnrolledUser enrolledUser = enrolledUserStudentMapping.getEnrolledUser(student);
			JSObject jsObject = new JSObject();
			jsArray.add(jsObject);
			jsObject.putWithQuote("id", enrolledUser.getId());
			jsObject.putWithQuote(FULLNAME, enrolledUser.getFullName());
			...
			jsObject.putWithQuote(CENTER, student.getCenter());
			jsObject.putWithQuote(SUBJECT, student.getSubject());
			jsObject.putWithQuote(SUBJECT_CREDITS, student.getSubjectCredits());
			jsObject.putWithQuote(TYPE_OF_TEACHING, student.getTypeOfTeaching());
			jsObject.putWithQuote(GROUP, student.getGroup());
			jsObject.putWithQuote(PERIOD, student.getPeriod());
			jsObject.putWithQuote(COURSE_YEAR, student.getCourseYear());
			jsObject.putWithQuote(TYPE_OF_SUBJECT, student.getTypeOfSubject());
			...
			jsObject.putWithQuote(TYPE_ACCESS_GRADE, student.getTypeAccessGrade());
			...
			jsObject.putWithQuote(STUDY_PLAN, student.getStudyPlan());
			...
			jsObject.putWithQuote(STUDY_PLAN_CREDITS, student.getStudyPlanCredits());
	...
	private JSArray createColumns() {
		...
		columns.add(createColumn(CENTER));
		columns.add(createColumn(SUBJECT));
		columns.add(createColumn(SUBJECT_CREDITS));
		columns.add(createColumn(TYPE_OF_TEACHING));
		columns.add(createColumn(GROUP));
		columns.add(createColumn(PERIOD));
		columns.add(createColumn(COURSE_YEAR));
		columns.add(createColumn(TYPE_OF_SUBJECT));
		columns.add(createColumn(YEARS_CONSUMED));
		columns.add(createColumn(NUMBER_OF_ENROLS));
		columns.add(createColumn(TYPE_ACCESS_GRADE));
		...
		columns.add(createColumn(STUDY_PLAN));
		...
		columns.add(createColumn(STUDY_PLAN_CREDITS));
